...
window.onload = function(){
    ...
    regForm.on('submit',function(){
        ...
        if(pw.val().trim().length==0){
            $('#msgPw').innerHTML='비밀번호는 필수항목입니다.';
            return false;
        } else if(!(pw.val().trim().length>=4&&pw.val().trim().length<=10)){
            $('#msgPw').innerHTML='비밀번호는 4~10자리까지 가능.';
            return false;
        }
        ...
        if(repw.val().trim().length==0){
            $('#msgRePw').innerHTML='비밀번호 확인은 필수입니다.';
            return false;
        } else if (repw.val()!=pw.val()){
            $('#msgRePw').innerHTML='비밀번호와 일치하지 않습니다.';
            return false;
        }
